...
class VisualServoConfig:
    ...
    def max_rotation_correction(self) -> float:
        ...
        return self.config.get('visual_servo.safety_limits.max_rotation', 0.175)  # ~10 degrees
    
    @property
    def max_total_correction(self) -> float:
        """Maximum total correction across all iterations (meters)"""
        return self.config.get('visual_servo.safety_limits.max_total_correction', 0.1)
